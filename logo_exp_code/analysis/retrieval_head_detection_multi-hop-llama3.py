...
class LLMNeedleHaystackTester:
    ...
    def __init__(
        ...
        save_contexts = True, final_context_length_buffer = 200, seconds_to_sleep_between_completions = None, print_ongoing_status = True,
        needle_file = None, insert_short_key_id = 0, needle_id = 0, shortcut_strategy = "random", peft_model_path = None
    ):
    ...
    def load_model(self):
        ...
        elif 'llama-3-8b' in self.model_version.lower():
            ...
            for head_idx in range(self.head_num):
                ...
                for v, i in zip(values, idx):  # find the top 1 attention id for the needle hit
                    ...
                    if is_i_in_needle_pos and inp.item() == self.prompt_ids[i].item():
                        attribution_scores[layer_idx][head_idx][0] += 1/(needle_ed1+needle_ed2-needle_st2-needle_st1)
    ...
    def find_needle_idx(self, needle):
        ...
        for i in range(len(self.prompt_ids)):            
            ...
            if(overlap > 0.9): needle_pos.append((i, i + span_len))
        return needle_pos
    ...
    def find_citation_idx(self, reference):
        ...
        for needle in reference:
            ...
            for i in range(len(self.prompt_ids)):            
                ...
                span_ids = set(token_span.tolist())
                overlap = float(len(span_ids.intersection(set(needle_ids)))) / len(set(needle_ids))
                if(overlap > 0.9): citation_pos.append((i, i + span_len))
    ...
    def evaluate_and_log(self, context_length, depth_percent):
        ...
        elif self.model_version.lower() == "llama3-8b-80k":
            messages = [{"role": "user", "content": context}]
            input_ids = self.enc.apply_chat_template(messages, tokenize=True, add_generation_prompt=True, return_tensors="pt", return_dict=True)
        else:
        ...
        initial_recall_score, penality_recall_score = self.scorer.score(response)
        ...
        if self.print_ongoing_status:
            ...
            print (f"score: {initial_recall_score}")
            print (f"penalized_recall_score: {penality_recall_score}")
    ...
    def encode_text_to_tokens(self, text):
        if self.model_provider in ["OpenAI", "LLaMA", "Mistral", "GLM", "llama3-8b-80k"]:
            return self.enc.encode(text)
        elif self.model_provider == "Anthropic": # Assuming you have a different encoder for Anthropic
    ...
    def insert_needle(self, context, depth_percents, context_length): 
        ...
        if self.model_provider in ["LLaMA", "LongLLaMA", "llama3-8b-80k"]: period_tokens = [29889, 869]
