CUDA_VISIBLE_DEVICES=7 python ./eval_needle.py \
    --max_length 64000 \
    --num_length_interval 15 \
    --min_length 8000 \
    --haystack_path "/vepfs/wcf/G/zecheng/Retrieval_Head/iclr2025/analysis/PaulGrahamEssays" \
    --result_dir "./results/llama3-pose" \
    --rouge rouge-1 \
    --eva_indic r \
    --chat_template 'no' \
    --model_path "/vepfs/wcf/G/zecheng/hf_models/Meta-Llama-3-8B-Instruct" \
    --peft_path "/vepfs/wcf/G/zecheng/ckpt/pose_chunk_size_1024/checkpoint-1000/context_scaling" \
    --max_position_embeddings 81920 --rope_theta 200e6;
...
# bash scripts/eval.sh 7 /vepfs/wcf/G/zecheng/hf_models/Meta-Llama-3-8B-Instruct 
