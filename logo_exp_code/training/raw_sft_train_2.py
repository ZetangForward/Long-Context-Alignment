...
class CustomDataCollator:
    ...
    def __call__(self, batch):
        ...
        inputs = self.tokenizer(question, padding=True, truncation=True, max_length=16384, return_tensors="pt")
        labels = self.tokenizer(answer, padding=True, truncation=True, max_length=2048, return_tensors="pt")

        input_ids = torch.concat([inputs.input_ids, labels.input_ids], dim=-1)
        attention_mask = torch.concat([inputs.attention_mask, labels.attention_mask], dim=-1)
        labels = torch.concat([torch.empty_like(inputs.input_ids).fill_(-100), labels.input_ids], dim=-1)
...
def train():
    ...
    train_dataset = auto_read_data(training_args.dataset_path)
    data_collator = CustomDataCollator(tokenizer, max_seq_len=20000)
