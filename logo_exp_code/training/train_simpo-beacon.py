...
class CustomDataCollator:
    def __call__(self, batch):
        ...
        if len(batch) == 1:
            wrap_batch["all_spe_pos"] = torch.tensor(batch[0]["all_spe_pos"]).unsqueeze(0)
...
class SimPOTrainer(DPOTrainer):
    ...
    def concatenated_forward(self, model, batch): # one chosen, two rejected
        ...
        # extract the logits of the special tokens
        all_spe_pos = concatenated_batch["all_spe_pos"][:, :-1]
        batch_indices = torch.arange(all_spe_pos.shape[0]).unsqueeze(1)
        selected_logits = all_logits[batch_indices, all_spe_pos]
        expanded_pos = all_spe_pos.repeat(3, 1)
        selected_logits = torch.gather(all_logits, 1, expanded_pos.unsqueeze(2))
