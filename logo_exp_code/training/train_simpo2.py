    ...
    def __call__(self, batch):
        ...
        if len(batch) == 1:
            ...
            for i in range(len(batch)):
                chosen_input_ids, chxosen_attention_mask, chosen_position_ids, chosen_labels, prefix_rejected_input_ids, prefix_rejected_attention_mask, prefix_rejected_position_ids, prefix_rejected_labels, suffix_rejected_input_ids, suffix_rejected_attention_mask, suffix_rejected_position_ids, suffix_rejected_labels = self.inner_wrap_batch(batch, i)
...
class SimPOTrainer(DPOTrainer):
    ...
    def concatenated_forward(self, model, batch): # one chosen, two rejected
        ...
        len_chosen = batch['chosen_labels'].size(0)
