...
# CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 ACCELERATE_LOG_LEVEL=info accelerate launch --deepspeed \
#     --mixed_precision bf16 --num_processes=8 

CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 deepspeed --num_gpus 8 training/train_simpo.py \
    --output_dir="/vepfs/wcf/G/zecheng/ckpt/simpo_vicuna_64k" \
    --max_steps=3000 \
    --logging_steps=1 \
    ...
    --learning_rate=1e-4 \
    --model_name_or_path "/vepfs/wcf/G/zecheng/hf_models/vicuna-7b-v1.5-16k" \
    ...
    --factor 16;
        ...
        # --deepspeed="training/config/stage2.json" \