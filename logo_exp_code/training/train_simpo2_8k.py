...
if __name__ == "__main__":
    ...
    if script_args.rope_type is not None:
    ...
    if script_args.peft_model_path is not None:
        model = PeftModelForCausalLM.from_pretrained(model, script_args.peft_model_path, torch_dtype="auto")
    
    if not script_args.full_finetune:
        log_c("begin low rank training...")
    ...
    dev_dataset = auto_read_data(os.path.join(script_args.dataset_path, "valid.pkl"))
