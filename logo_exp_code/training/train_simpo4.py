...
class SimPOTrainer(DPOTrainer):
    ...
    def create_random_position_ipt_data(self, feature):
        # We create positional embeddings totally offline
        pass
    ...
    def concatenated_forward(self, model, batch): # one chosen, two rejected
        ...
        # concate input_ids and answer into one tensor
        ...
        # all_logits = model(wrap_input_ids, attention_mask=wrap_attention_mask, position_ids=wrap_position_ids, use_cache=False, return_dict=True).logits

        # normal CE Loss (only compute right part of the logits)
        # shift_logits = all_logits[0, :-1, :].contiguous()
        # shift_labels = batch['chosen_answer']['labels'][..., 1:].contiguous()
        # # Flatten the tokens
        ...
        hidden_states = model(concatenated_batch["input_ids"], attention_mask=concatenated_batch["attention_mask"], position_ids=concatenated_batch["position_ids"], use_cache=True, return_dict=True)
...
if __name__ == "__main__":
    ...
    base_model, tokenizer = FastLanguageModel.from_pretrained(
        ...
        max_seq_length = script_args.max_position_embeddings,
        dtype = torch.bfloat16,
        device_map=None,
        load_in_4bit = script_args.load_in_4_bit,
        token=script_args.access_token,
        rope_theta=script_args.rope_theta,
    )
    ...
    model = FastLanguageModel.get_peft_model(
        ...
        modules_to_save=[script_args.lora_extra_params],
    ...
    print(f"Trainable Model params: {format_numel_str(sum(p.numel() for p in model.parameters() if p.requires_grad))}")
